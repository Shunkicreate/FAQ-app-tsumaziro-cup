...
const FAQList = ({ items, color, hoverColor, input }: FAQListProps): JSX.Element => {
    return (
        <>
            {
                input === "" ? <span className="text-[#2B546A] text-base">
                    Frequently Asked Questions
                </span> : <span className="text-[#2B546A] text-base">{`${items.length} questions matched`}</span>
...
const ShowUnorderedList = (props: { items: FAQ[], input: string }): JSX.Element => {
    const { items, input } = props;
    const color = useColorModeValue("#2B546A", "gray.300");
    const hoverColor = useColorModeValue("gray.200", "gray.900");
    return <FAQList items={items} color={color} hoverColor={hoverColor} input={input}/>;
};
